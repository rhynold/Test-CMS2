...
function classic_theme_setup() {
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'classic_theme' ),
		)
	);
...
function classic_theme_widgets_init() {
	register_sidebar(
		array(
			...
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'classic_theme' ),
...
function classic_theme_scripts() {
	wp_enqueue_style( 'classic_theme-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'classic_theme-style', 'rtl', 'replace' );

	wp_enqueue_script( 'classic_theme-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
